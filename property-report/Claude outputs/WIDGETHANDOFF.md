...
6. **`.gitignore`** and **`.npmignore`**: exclude `node_modules`, `.vs`, and OS cruft.
7. **Never ship** `node_modules` or the `.vs` Visual Studio cache folder. Be aware that
...
2. Add the standardized files to it: `package.json`, `README.md`, `LICENSE`,
   `.gitignore`, `.npmignore` (see Section 2). The README must state the exact folder
...
3. **Visual Studio errors that webpack does not have.** Webpack (`npm start` in `client`) is
   ...
   **Mode B, self-contained (VS reports IDE1100 on node_modules; the September 2026 state
   ...
   `skipLibCheck: true`, `lib: ["ESNext", "DOM", "DOM.Iterable"]`, `jsx: "react-jsx"` with
   `jsxImportSource: "@emotion/react"`, `include: ["src/**/*.ts", "src/**/*.tsx",
